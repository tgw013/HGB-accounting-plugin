...
**Status:** `v2.7.2`. 16 Skills + 16 Commands, 128 Konten gegen DATEV-PDFs 2025+2026 verifiziert, **DATEV-EXTF-Export deterministisch** (Python-Serializer mit PORTAL-verifizierter Feld-Inventar; identische Eingabe → byte-identische CSV via SHA-256, GoBD-relevant) + **Automatikkonten-Schutz** (verhindert REW00305) + **Wiederkehrende Buchungen** (Formatkategorie 65 v4 für monatliche/quartalsweise Serien) + **KOST-Splitt** (eine Buchung über mehrere Kostenstellen verteilen via `kost_allocations`) + **buchungssatz erweitert** (Belege + Abschlussbuchungen § 249/§ 250, JSON-Handoff für datev-export) + **bwa-kommentierung** (BWA-/GuV-Kommentierung, Pendant zu `financial-statements`) + **EXTF-Saldo-Fix** (vollständige Konto-an-Gegenkonto-Zeilen self-balancing → keine Doppelbuchung beim Import). Vorgänger-Tags: `v1.1.0`, `v2.0.0-alpha`, `v2.0.0`, `v2.0.1`, `v2.1.0`, `v2.3.0`, `v2.5.0`, `v2.6.0`, `v2.7.0`, `v2.7.1`.
